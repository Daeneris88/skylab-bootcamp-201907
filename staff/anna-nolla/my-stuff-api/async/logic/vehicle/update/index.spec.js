...
describe('logic - update vehicle', () => {
    ...
    it('should succeed on correct data', async () => {
        const result = await logic.updateVehicle(userId1, plate , updatedData)
    ...
    it('should fail on non-existing user', async () => {
        ...
        try {
            await logic.updateVehicle(userId1, plate , updatedData)
    ...
    it('should fail on non-existing vehicle', async () => {
        try{
            await logic.updateVehicle(userId1, '123' , updatedData)
            throw new Error('should not reach this point') 
        }catch({ message }) {
            expect(message).to.equal(`vehicle with plate 123 does not exist`)
        }
    })
