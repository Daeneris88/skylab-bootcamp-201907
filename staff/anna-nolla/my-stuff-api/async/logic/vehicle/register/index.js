...
module.exports = function(id, make, model, year, type, color, electric, plate) {
    ...
    return(async () => {
        const response = await Vehicle.findOne({ plate })
            ...
            const vehicle = await new Vehicle({ make, model, year, type, color, electric, plate })
            vehicle.owner.push(id)
            await vehicle.save()
        const res = await Vehicle.findOne({ plate })
            if (!res) throw new Error(`Vehicle with plate ${plate} does not exist`)
